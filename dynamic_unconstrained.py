...
Algorithms for the dynamic unconstrained reassignment problem:
    ...
    - The number of columns that can be reassigned is unconstrainted
"""
...
# Optimal unconstrained reassignment through MILP
def reassign(
        workload: Workload,
    ...
    """
    (Depreciated) One to many formulation:
    Goal: minimize L
    Constraints:
...
if __name__ == "__main__":
    ...
    os.makedirs(f"test/uncon_MIQCP/c{res}_p{procs}", exist_ok=True)
    assignment = reassign(workload, original_assignment, 0, 1,
                           f"test/uncon_MIQCP/c{res}_p{procs}/solution.txt", 10000.0)
    assignment.write_csv(f"test/uncon_MIQCP/c{res}_p{procs}/assignment.csv")
