...
def reshape_to_2d(matrix_3d):
    height, width, interval = len(matrix_3d), len(
        matrix_3d[0]), len(matrix_3d[0][0])
...
def coarsen(matrix_2d, width, height, interval, coarse_constant):
    ...
    coarse_matrix = [
        [[0]*interval for _ in range(coarse_height)] for _ in range(coarse_width)]
    new_dictionary = {}
    ...
    for y in range(height):
        ...
        for x in range(width):
            ...
            new_dictionary[whtoi(i, j, coarse_width, coarse_height)].append(
                tuple_value)
            for k in range(interval):
                coarse_matrix[i][j][k] += matrix_2d[whtoi(
                    x, y, width, height)][k]
...
def greedy(matrix: list, width: int, height: int, intervals: int, processors: int, extra: int) -> List[int]:
    ...
    for sample in samples_list:
        ...
        for i in range(intervals):
            ...
            last_costs[i] = max(
                last_costs[i], processor_costs[min_processor][i])
    ...
    for sample in samples_list:
        ...
        if trivial:
            ...
            for i in range(intervals):
                ...
                last_costs[i] = max(
                    last_costs[i], processor_costs[original_processor][i])
...
def dynamic_reassignment(matrix: pd.DataFrame, width: int, height: int, processors: int, original_assignment: list, reassignment_cost: float, lower_ratio: float, upper_ratio: float) -> List[int]:
    ...
    sorted_processors = sorted(
        range(processors), key=lambda x: processor_costs[x])
...
def limited_dynamic(matrix: pd.DataFrame, width: int, height: int, processors: int, original_assignment: list, iterations: float, lower_ratio: float, upper_ratio: float) -> List[int]:
    ...
    for i in range(len(sorted_processors) // 2):
        ...
        for column, cost in processor_columns[sorted_processors[indices[1]]].items():
            itemized_columns.append((column, cost))
        # sort the columns by their costs
        itemized_columns = sorted(itemized_columns, key=lambda x: x[1], reverse=True)
        # for each column, assign it to the processor with the least cost until one of the processors fills up
        last_column = -1
        remaining_processor = -1
...
def balance_greedy(matrix: list, width: int, height: int, intervals: int, processors: int, original_assignment: list, reassignment_cost: float) -> List[int]:
    ...
    for sample in range(samples):
        ...
        for i in range(intervals):
            processor_costs[original_assignment[sample]
                            ][i] += matrix[sample][i]
            processor_total_costs[original_assignment[sample]
                                  ] += matrix[sample][i]

    # sort the processors by their total costs
    sorted_processors = sorted(
        processor_total_costs.items(), key=lambda x: x[1])
    ...
    while True:
        ...
        if most_costly[1] - least_costly[1] <= reassignment_cost * intervals:
            print('Stopping reassignment as the difference between {}: {} and {}: {} is less than {}'.format(
                most_costly[0], most_costly[1], least_costly[0], least_costly[1], reassignment_cost * intervals))
            break
        ...
        new_cost = (most_costly[1] + least_costly[1] +
                    reassignment_cost * intervals) / 2
    ...
    for processor in range(processors):
        ...
        if reassignment_targets[processor]:
            ...
            for reassignment_target in reassignment_targets[processor]:
                for i in range(intervals):
                    last_costs[i] = max(
                        last_costs[i], processor_costs[reassignment_target][i])
            ...
            for sample in processor_cells[processor]:
                ...
                for reassignment_target in reassignment_targets[processor]:
                    ...
                    for i in range(intervals):
                        costs[reassignment_target] += max(
                            last_costs[i], processor_costs[reassignment_target][i] + matrix[sample][i])
...
def reshuffle_greedy(matrix: list, width: int, height: int, intervals: int, processors: int, original_assignment: list, reassignment_cost: float) -> List[int]:
    ...
    for i in range(processors - 1):
        for j in range(i + 1, processors):
            ...
            cost_difference = sum(
                abs(processor_costs[i][k] - processor_costs[j][k]) for k in range(intervals))
    ...
    for group in reshuffle_groups:
        ...
        for sample in unassigned_samples:
            ...
            for processor in group:
                costs[processor] = 0
                for i in range(intervals):
                    costs[processor] += max(last_costs[i],
                                            processor_costs[processor][i] + matrix[sample][i])
            # assign the sample to the processor with the minimum cost
            min_processor = min(costs, key=costs.get)
            # update the assignments
            assignments[sample] = min_processor
...
def greedy_with_communication(matrix: list, width: int, height: int, intervals: int, processors: int, extra: int, send_cost: float, recv_cost: float, cost_function) -> List[int]:
    ...
    for sample in samples_list:
        ...
        secondary_neighbors = [left_left, left_up, up_up,
                               right_up, right_right, right_down, down_down, left_down]
        ...
        for processor in range(processors):
            ...
            temp_cost = cost_function(matrix, assignments, sample, processor, temp_processor_costs, temp_communication_costs,
                                      temp_already_broadcasted, neighbors, secondary_neighbors, intervals, processors, send_cost, recv_cost)
...
def greedy_prioritize_communication_fine_tune(matrix: list, width: int, height: int, intervals: int, processors: int, extra: int, send_cost: float, recv_cost: float, constant: float, coarse: int, single_entry: bool) -> List[int]:
    ...
    processor_workload_array = np.zeros(
        processors * intervals).reshape(processors, intervals)
    workload_array = np.array(matrix)
    num_rows = len(matrix)
    num_cols = len(matrix[0]) if matrix else 0
    computation_overload_threshold = constant * \
        np.sum(workload_array, axis=0).reshape(-1, 1)/processors
    coarsen_workload_array, coarse_width, coarse_height, coarse_dictionary = coarsen(
        workload_array, width, height, intervals, coarse)
    ...
    for coarsen_sample in coarsen_samples_list:
        ...
        for ranked_processor in ranked_array:
            threshold = copy.deepcopy(processor_workload_array)[
                ranked_processor].reshape(-1, 1)
        ...
        if min_processor != -1:
            ...
            if (single_entry):
                ...
                for sample in samples:
                    ...
                    for processor in range(processors):
                        # uncoarsen the block
                        tmp_processor_workload_array = copy.deepcopy(
                            processor_workload_array)
                        tmp_processor_workload_array[processor] += workload_array[sample]
                        tmp_cost = np.sum(
                            np.max(tmp_processor_workload_array, axis=0))
                ...
                for sample in samples:
                    assignment[sample] = min_processor
        for sample in samples:
            np.add(processor_workload_array[min_processor],
                   workload_array[sample], out=processor_workload_array[min_processor])
...
def greedy_prioritize_communication(matrix: list, width: int, height: int, intervals: int, processors: int, extra: int, send_cost: float, recv_cost: float, constant: float) -> List[int]:
    ...
    for sample in samples_list:
        ...
        ranked_array = list(dictionary.keys())
        for ranked_processor in ranked_array:
            tmp_array = processor_workload_array[ranked_processor].reshape(
                -1, 1) + workload_array[sample].reshape(-1, 1)
            is_overloaded = False
...
def mosaic_greedy(matrix: list, width: int, height: int, intervals: int, processors: int, extra: int, constant: float, coarse: int) -> List[int]:
    ...
    for i in range(intervals):
        threshold[i] = constant * sum(matrix[sample][i]
                                      for sample in range(samples)) / processors
    ...
    for sample in samples_list:
        ...
        while tentative_block_size <= coarse:
            ...
            for processor in range(processors):
                ...
                for i in range(intervals):
                    # the new cost of this processor at this interval
                    interval_costs[i] = processor_costs[processor][i] + \
                        block_costs[i]
        ...
        for i in range(intervals):
            ...
            last_costs[i] = max(
                last_costs[i], processor_costs[tentative_block_processor][i])
    ...
    for block_size in block_sizes:
        print('{} {} {}'.format(block_size,
              block_sizes[block_size], block_counts[block_size]))
...
def blocked_greedy(matrix: list, width: int, height: int, intervals: int, processors: int, extra: int, constant: float, coarse: int) -> List[int]:
    ...
    for block in range(blocks):
        ...
        for processor in range(processors):
            for i in range(intervals):
                # the new cost of this processor at this interval
                cost = processor_costs[processor][i] + \
                    blocks_list[block].cost[i]
                # the cost of this interval is the max of the last cost and the new cost
                costs[processor] += max(last_costs[i], cost)
        # assign to the leftmost processor with the lowest cost
        ...
        # update the processor costs and last costs
        for i in range(intervals):
            processor_costs[block_assignments[block]
                            ][i] += blocks_list[block].cost[i]
            last_costs[i] = max(
                last_costs[i], processor_costs[block_assignments[block]][i])
