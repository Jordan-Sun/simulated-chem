...
def minimize_difference(pairs):
    N = len(pairs)
    d = [p - q for p, q in pairs]
    ...
    for i in range(N, 0, -1):
        ...
        if (s - di) in dp[i - 1]:
            # Assigned +1
            s -= di
            A.append(pairs[i - 1][0])
            B.append(pairs[i - 1][1])
            swapped.append(False)
        else:
            # Assigned -1
            s += di
            A.append(pairs[i - 1][1])
            B.append(pairs[i - 1][0])
            swapped.append(True)

    # The reconstruction was done backwards
    A = A[::-1]
    B = B[::-1]
    swapped = swapped[::-1]
...
start = time.time()
A, B, swapped = minimize_difference(pairs)
end = time.time()
print("Time:", end - start)
print("A:", A)
print("B:", B)
print("Swapped:", swapped)
print("Difference:", sum(A) - sum(B))
