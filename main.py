...
def process_interval(interval: int, root: str, bias: float = 0.0, mem: float = None) -> Assignment:
    ...
    with open(f"{root}/logs/{interval}.txt", 'w') as f:
        with contextlib.redirect_stdout(f):
            ...
            assignment = dynamic_constrained.MIQCP(
                workload, original_assignment, interval, 1, f"{root}/solutions/{interval}.txt", True, bias, mem)
...
if interval is None:
    ...
    with multiprocessing.Pool(processes=os.cpu_count()) as pool:
        assignments = pool.map(partial(process_interval, root=root, bias=bias, mem=proc_mem), range(workload.intervals), chunksize = 1)
    ...
    _ = dynamic_constrained.MIQCP(
        workload, original_assignment, interval, os.cpu_count(), f"{root}/solutions/{interval}.txt", False, bias, total_mem)
